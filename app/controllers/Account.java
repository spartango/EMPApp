...
public class Account extends Controller {
    ...
    public static class Accept {
        ...
        @Required @NonEmpty public Boolean accept;

    }

    public static class PasswordChange {
        @MinLength(5) @Required public String password;

        @MinLength(5) @Required public String repeatPassword;
    ...
    @RoleHolderPresent public static Result link() {
        return ok(link.render());
    }

    @Restrict(Application.USER_ROLE) public static Result verifyEmail() {
        ...
        if (user.emailValidated) {
            ...
            flash(Application.FLASH_MESSAGE_KEY,
                  Messages.get("playauthenticate.verify_email.message.instructions_sent",
                               user.email));
            MyUsernamePasswordAuthProvider.getProvider()
                                          .sendVerifyEmailMailingAfterSignup(user,
                                                                             ctx());
        } else {
            flash(Application.FLASH_MESSAGE_KEY,
                  Messages.get("playauthenticate.verify_email.error.set_email_first",
                               user.email));
        }
        return redirect(routes.Application.profile());
    }

    @Restrict(Application.USER_ROLE) public static Result changePassword() {
    ...
    @Restrict(Application.USER_ROLE) public static Result doChangePassword() {
    ...
    @RoleHolderPresent public static Result askLink() {
    ...
    @RoleHolderPresent public static Result doLink() {
        final AuthUser u = PlayAuthenticate.getLinkUser(session());
        if (u == null) {
            // account to link could not be found, silently redirect to login
            return redirect(routes.Application.index());
    ...
    @RoleHolderPresent public static Result askMerge() {
    ...
    @RoleHolderPresent public static Result doMerge() {
        // this is the currently logged in user
        final AuthUser aUser = PlayAuthenticate.getUser(session());
