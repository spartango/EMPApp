...
public class MyUsernamePasswordAuthProvider extends
    ...
    @Override protected SignupResult
    ...
    @Override protected LoginResult
    ...
    @Override protected Call
            userExists(final UsernamePasswordAuthUser authUser) {
        return routes.Signup.exists();
    }

    @Override protected Call
            userUnverified(final UsernamePasswordAuthUser authUser) {
        return routes.Signup.unverified();
    }

    @Override protected MyUsernamePasswordAuthUser
            buildSignupAuthUser(final MySignup signup, final Context ctx) {
        return new MyUsernamePasswordAuthUser(signup);
    }

    @Override protected MyLoginUsernamePasswordAuthUser
            buildLoginAuthUser(final MyLogin login, final Context ctx) {
        return new MyLoginUsernamePasswordAuthUser(login.getPassword(),
                                                   login.getEmail());
    }

    @Override protected String
            getVerifyEmailMailingSubject(final MyUsernamePasswordAuthUser user,
                                         final Context ctx) {
    ...
    @Override protected Body
            getVerifyEmailMailingBody(final String token,
        ...
        final String html = views.html.account.signup.email.verify_email.render(url,
                                                                                token,
                                                                                user.getName())
                                                                        .toString();
        final String text = views.txt.account.signup.email.verify_email.render(url,
                                                                               token,
                                                                               user.getName())
                                                                       .toString();
    ...
    @Override protected String
            generateVerificationRecord(final MyUsernamePasswordAuthUser user) {
